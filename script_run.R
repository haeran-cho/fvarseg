...
for(S in c(7:18)){ 
  ...
  for(i in 1:N){
    ...
    ### 1) data generation 
    ss <- sim.data0(n, p, q,  
                    cp.common = cp.common, den.common = .5, type.common = type.common, ma.order = 2,
                    cp.idio = cp.idio, size.idio = 1, do.scale = TRUE, seed = i)
      
    x <- ss$x
    #x <- ss$xi
    
    ### 2) common component 
    cs <- common.seg(x, G.seq = NULL, thr = NULL, tt.by = ceiling(log(dim(x)[2])), 
    ...
    cat("model", S, " / run", i, 
        " / est.cp.com: ", sapply(est.cp.com[[i]], paste, collapse=' '), 
        " / est.cp.idio: ", sapply(est.cp.idio[[i]], paste, collapse=' '))
    cat("\n")
  }
  
  result <- list(est.cp.com = est.cp.com, est.cp.idio = est.cp.idio,
                 cp.common = cp.common, cp.idio = cp.idio, n = n)
  #save(result, file=paste0("S", S, "_simdat0_den05_sizeidio1_etaeta_N", N, ".RData", sep = ""))
...
###################################################################################
###################################################################################
### chi exists without changes
sim.list = expand.grid(structure(list( cp.idio = list(c(round(n * c(3,5)/8))), 
                                       p = c(50, 100, 150), 
...
for(S in 1:dim(sim.list)[1]){
  ...
  result <- list(est.cp.com = est.cp.com, est.cp.idio = est.cp.idio,
                 cp.common = cp.common, cp.idio = cp.idio, n = n)
  #save(result, file=paste0("S", S, "_nocpINchi_den05_sizeidio1_etaeta_N", N, ".RData", sep = ""))
